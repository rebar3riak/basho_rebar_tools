...
dirty({Name, Repo, _} = App, {Long, _} = Ctx, _State) ->
    ...
    case rrp_repo:dirty(Repo) of
        ...
        {true, OutLines} ->
            case Long of
                true ->
                    ?LOG_WARN("Repository dirty: ~s~n~s",
                        [Name, string:join(OutLines, "\n")]),
                    {ok, {Long, false}};
                _ ->
                    ?LOG_WARN("Repository dirty: ~s", [Name]),
                    {ok, {Long, false}}
            end;
        Error ->
...
pull({Name, Repo, _} = App, {Long, _Result} = Ctx, _State) ->
    ...
    case rrp_repo:pull(Repo) of
        {ok, OutLines} ->
            case Long of
                true ->
                    ?LOG_INFO("Repository pulled: ~s~n~s",
                        [Name, string:join(OutLines, "\n")]),
                    {ok, Ctx};
                _ ->
                    ?LOG_INFO("Repository pulled: ~s", [Name]),
                    {ok, Ctx}
            end;
        Error ->
            format_error(App, Error)
...
version({Name, Repo, _} = App, {Long, _} = Ctx, _State) ->
    ...
    case rrp_repo:version(Repo) of
        ...
        {error, _} = Error ->
            format_error(App, Error);
        {Type, Vsn} ->
            ?LOG_INFO("Repository ~s: {~s, \"~s\"}", [Name, Type, Vsn]),
            {ok, Ctx}
    end.
