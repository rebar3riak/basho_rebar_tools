...
%%
%% @doc RRP provider for the `rrp-rc' command.
%%
-module(rrp_prv_rc).
...
-define(PROVIDER_ATOM,  'rrp-rc').
-define(PROVIDER_STR,   "rrp-rc").
...
-define(PROVIDER_OPTS,  [
    ...
    {skip, $s, "skip", boolean,
        ...
        "files whose rrp.protect flag disallows updates. "
...
do(State) ->
    ...
    ResetVersion = case proplists:get_value(rebar2, Opts) of
        ...
        _ ->
            rrp_rebar:config_format(default)
    end,
    FileNames = rrp_rebar:cfg_file_names(),
                ...
                true ->
                    case FileNames of
                        [Name | _] ->
                            FileNames;
                        _ ->
                            rrp_rebar:cfg_file_names(
                                [Name | lists:delete(Name, FileNames)])
                    end;
                _ ->
                    rrp_rebar:cfg_file_names([Name | FileNames])
            end
    end,
    Result = handle_command(Opts, State),
    _ = rrp_rebar:config_format(ResetVersion),
    _ = rrp_rebar:cfg_file_names(ResetNames),
    Result.
...
-spec all_deps(CalcDeps :: [rrp:app_name()]) -> [rrp:app_name()].

all_deps(CalcDeps) ->
    Forced  = rrp:get_key_list(forced, rrp_config:config()),
    lists:usort(lists:append(Forced, CalcDeps)).

-spec create_rebar_config(
    File        :: rrp:fs_path(),
    App         :: rrp:app_spec(),
    XrefDeps    :: [rrp:app_name()],
    CpyInfo     :: current | rrp:basho_year() | iolist(),
    WarnBlocked :: boolean())
        -> ok | rrp:prv_error().
...
create_rebar_config(File, {Name, _, _} = App, XrefDeps, CpyInfo, WarnBlocked) ->
    ...
    TstDeps = lists:subtract(rrp_fudge:test_deps(App), AllDeps),
    Config  = rrp_defaults:rebar_config(Name, PrdDeps, TstDeps, []),
...
handle_command(Opts, State) ->
    ...
    case rrp_xref:new(State) of
        {ok, XRef} ->
            ...
            Result = rrp_rebar:fold(Select, fun update/3, Context, State),
            rrp_xref:stop(XRef),
            case Result of
                {ok, _} ->
                    {ok, State};
    ...
    case rrp_xref:app_deps(XRef, Name) of
        {ok, XrefDeps} ->
            ...
            case update_rebar_config(
                    rrp:find_first(file, FileNames, [Path]),
...
update_rebar_config(
    ...
    case rrp_rebar:copyright_info(FileIn, Loose, erl) of
        {error, _} = CpyErr ->
            CpyErr;
        CpyInfo ->
            case DepsOnly of
...
write_rebar_config(File, Config, CpyInfo, WarnBlocked) ->
    case rrp_config:overwrite_blocked(File) of
        true ->
            case WarnBlocked of
                true ->
                    ?LOG_WARN("~s: ~s",
        ...
        _ ->
            case file:open(File, [write]) of
                {ok, IoDev} ->
                    Result = rrp_io:write_rebar_config(IoDev, Config, CpyInfo),
                    _ = file:close(IoDev),
                    Result;
                {error, What} ->
